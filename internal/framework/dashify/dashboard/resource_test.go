...
func TestDashboardBlocksSchema(t *testing.T) {
	t.Parallel()

	blocks := Blocks()
	controlBar, ok := blocks["control_bar"].(schema.SingleNestedBlock)
	require.True(t, ok)
	...
	rootLayout, ok := blocks["layout"].(schema.SingleNestedBlock)
	require.True(t, ok)
	...
	rootContainer, ok := blocks["container"].(schema.ListNestedBlock)
	require.True(t, ok)
	...
	for _, name := range []string{
		charts.MetricsTimeSeriesName,
		charts.MetricsSingleValueName,
		charts.MetricsListName,
		charts.MetricsTableName,
		charts.MetricsClusterMapName,
		charts.TextName,
	} {
		_, isSingleNestedBlock := rootContainer.NestedObject.Blocks[name].(schema.SingleNestedBlock)
		assert.True(t, isSingleNestedBlock, "root container chart %q must be a single nested block", name)
	}
	timeSeries := rootContainer.NestedObject.Blocks[charts.MetricsTimeSeriesName].(schema.SingleNestedBlock)
	_, ok = timeSeries.Attributes["series"].(schema.MapNestedAttribute)
	assert.True(t, ok, "time-series per-series overrides must be a nested map attribute")
	_, ok = timeSeries.Attributes["y_axes"].(schema.ListNestedAttribute)
	assert.True(t, ok, "time-series axes must be a nested list attribute")
	singleValue := rootContainer.NestedObject.Blocks[charts.MetricsSingleValueName].(schema.SingleNestedBlock)
	_, ok = singleValue.Attributes["display_unit"].(schema.SingleNestedAttribute)
	assert.True(t, ok, "display_unit must be a nested attribute")
	_, ok = singleValue.Attributes["color_scale"].(schema.ListNestedAttribute)
	assert.True(t, ok, "color_scale must be a nested list attribute")
	listChart := rootContainer.NestedObject.Blocks[charts.MetricsListName].(schema.SingleNestedBlock)
	_, ok = listChart.Attributes["published_streams"].(schema.MapNestedAttribute)
	assert.True(t, ok, "published_streams must be a nested map attribute")
	_, ok = listChart.Attributes["sort"].(schema.SingleNestedAttribute)
	assert.True(t, ok, "sort must be a nested attribute")
	...
	for _, name := range []string{
		...
		assert.Contains(t, sectionContainer.NestedObject.Blocks, name)
	}
	...
	for _, name := range []string{
		...
		assert.Contains(t, groupContainer.NestedObject.Blocks, name)
	}
}
...
func TestDashifyContainerContentErrorsDescribeOneContainer(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"dashboard": {
			...
			want:  "each dashboard container must set exactly one content block: template, metrics_cluster_map, metrics_list, metrics_single_value, metrics_table, metrics_time_series, text, section, or group",
		},
		"section": {
			level: dashifySectionContainerLevel,
			want:  "each container within a section must set exactly one content block: template, metrics_cluster_map, metrics_list, metrics_single_value, metrics_table, metrics_time_series, text, or group",
		},
		"group": {
			level: dashifyGroupContainerLevel,
			want:  "each container within a group must set exactly one content block: template, metrics_cluster_map, metrics_list, metrics_single_value, metrics_table, metrics_time_series, or text",
		},
	}
...
func TestDashifyDashboardInlineContentRoundTrip(t *testing.T) {
	...
	for name, content := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			// Refresh must leave this raw panel raw even though the element is a
			// recognized chart type. The document cannot record that choice, so
			// the no-op check is what preserves it: state already describes this
			// exact document, so Read does not reparse it into a typed block.
			root := template.RootElementDashboard
			...
			assert.True(t, MatchesModel(record, model), "stored document must match the raw model it was written from")
		})
	}
}
...
func TestDashifyDashboardSpecWarnsAboutUnmodeledFields(t *testing.T) {
	...
	spec := json.RawMessage(`{
		...
		"<Dashboard>":[
			{"<Section>":[
				{"<Group>":[
					{"<Panel>":[{"<$import.widget0>":[]}],"futureContainer":true,"futureContent":{"preserve":true}}
				]}
			]}
		],
		"layout":{"saved":{
