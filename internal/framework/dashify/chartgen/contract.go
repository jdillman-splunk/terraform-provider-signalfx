...
func loadContractDocument(path string) (contractDocument, string, error) {
	contents, err := os.ReadFile(path)
	if err != nil {
		return contractDocument{}, "", fmt.Errorf("reading Olly contract %s: %w", path, err)
	}
	var root map[string]any
	if err := json.Unmarshal(contents, &root); err != nil {
		return contractDocument{}, "", fmt.Errorf("parsing Olly contract %s: %w", path, err)
	}
	title, ok := root["title"].(string)
	if !ok || title == "" {
		return contractDocument{}, "", fmt.Errorf("olly contract %s has no string title", path)
	}
	defs, _ := root["$defs"].(map[string]any)
	return contractDocument{path: path, root: root, defs: defs}, title, nil
}
...
func loadContracts(directory string, charts []chartInfo) (map[string]*contractNode, error) {
	...
	for _, path := range paths {
		document, title, readErr := loadContractDocument(path)
		if readErr != nil {
			return nil, readErr
		}
		tag := "<" + title + ">"
		...
		for _, name := range sortedAnyKeys(document.defs) {
			definition, definitionOK := document.defs[name].(map[string]any)
			if !definitionOK {
		...
		node, compileErr := document.compile(document.root, "#", map[string]bool{})
		if compileErr != nil {
			return nil, compileErr
		}
		contracts[tag] = node
...
func (d contractDocument) compile(raw map[string]any, location string, resolving map[string]bool) (*contractNode, error) {
	...
	if rawReference, present := raw["$ref"]; present {
		...
		copy := *compiled
		copy.ReferenceName = name
		if description, present, err := contractStringAnnotation(raw, "description", d.path, location); err != nil {
			return nil, err
		} else if present {
	...
	if description, _, err := contractStringAnnotation(raw, "description", d.path, location); err != nil {
		return nil, err
	} else {
...
func contractStringAnnotation(raw map[string]any, key, path, location string) (string, bool, error) {
	...
	if !ok {
		return "", true, fmt.Errorf("%s %s has non-string %s annotation", path, location, key)
	}
