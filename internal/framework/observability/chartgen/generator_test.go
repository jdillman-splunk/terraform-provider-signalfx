...
func TestWriteAndCheckModesDetectEveryStaleStateWithoutMutation(t *testing.T) {
	...
	newFixture := func(t *testing.T) generationOptions {
		t.Helper()
		root := t.TempDir()
		copyTree(t, filepath.Join(repository, "internal", "framework", "observability", "charts", "schemas"), filepath.Join(root, "internal", "framework", "observability", "charts", "schemas"))
